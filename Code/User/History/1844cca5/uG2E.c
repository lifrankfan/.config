...
compress_deflate(const char *pci_addr, char *file_data, size_t file_size, enum doca_compress_job_types job_type, const char *output_path)
{
	...
	result = doca_compress_create(&compress); // pointer to pointer to compress
	if (result != DOCA_SUCCESS) {
	...
	state.ctx = doca_compress_as_ctx(compress); // pointer to compress
...
// allocates memory for an array of elements of max file size initialized to 0
