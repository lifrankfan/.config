...
// question: why need compress_create?; a pointer to pointer to compression data?
...
// memory map src and des	result = init_core_objects(&state, workq_depth, max_bufs);
	if (result != DOCA_SUCCESS) {
		fclose(out_file);
...
// question: destination buffer and source buffer are similar to array? contigious memory?
...
// question: where is job going exactly? to smartnic?
